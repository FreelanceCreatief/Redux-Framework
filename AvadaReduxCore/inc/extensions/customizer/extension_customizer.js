...
(function( $ ) {  //This functions first parameter is named $
    ...
    avadaredux.customizer.save = function( $obj, $container ) {
        ...
        avadaredux.customizer.inputSave( $parent );
    };
    avadaredux.customizer.inputSave = function( $parent ) {

        if ( !$parent.hasClass( 'avadaredux-field-container' ) ) {
            $parent = $parent.parents( '[class^="avadaredux-field-container"]' );
        }

        var $id = $parent.parent().find( '.avadaredux-customizer-input' ).data( 'id' );

        if ( !$id ) {
            $parent = $parent.parents( '.avadaredux-container-repeater:first' );
            var $id = $parent.parent().find( '.avadaredux-customizer-input' ).data( 'id' );
        }
        ...
        var $key = $parent.parent().find( '.avadaredux-customizer-input' ).data( 'key' );
        if ( $nData[$key] ) {
            $nData = $nData[$key];
        }
